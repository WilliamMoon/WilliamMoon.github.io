...
这篇文章以本人的视角介绍如何快速搭建一个静态博客网站。
<!--more-->
不过仅有展示文章的基本功能，更多功能比如评论系统、搜索功能，等后续做了再考虑完善相关内容。随着基础设施的发展，对我这样只是发发文章的人来说，未来不见得有独立建站的必要性，现在就是折腾着玩。
...
## 选择静态网站生成器
首先需要选择一个静态网站生成器，常见的有Hugo、Jekyll、Hexo等。三者我都尝试了下，Jekyll采用Ruby编写，是GitHub Pages官方支持的生成器，自带workflow，相当适合初学者使用，缺点是要装Ruby而且网站构建速度较慢。Hexo基于Node.js，适合有前端基础、喜欢定制化的用户，生成速度较快，缺点也是要装环境。Hugo采用编译型的Go语言编写，生成速度极快，能做到修改后毫秒级重生成，适合内容较多的网站。

我先选择了Hugo，因为它没有环境依赖，我电脑上少装一个是一个，而且Node.js和Ruby环境都有点臃肿。不过总结网友评论，最后要完全满足自己需求都要搞最重型的WordPress，二十多年的插件生态、60.5%的CMS市场占有率（根据[W3Techs](https://w3techs.com/technologies/details/cm-wordpress)最新数据）确实不是吹的。

## 安装Hugo
版本上我直接安装Hugo Extended，因为相比Standard版基本没有额外成本。有这个优点，具体的好处比如支持Sass从而支持更多主题、支持WebP图像处理都不用细看了。

我同时在使用多台电脑，Windows 10/11和macOS系统都有，并且每个系统都安装了包管理器，在每个系统下安装Hugo都非常简单，一个命令搞定。其他系统和其他安装方式参见[Hugo官方文档](https://gohugo.io/installation/)。
...
## 站点初始化
在站点目录（本文以`hugo-blog`为例）上级目录执行以下命令初始化站点，添加主题
```sh
...
文章开头的元信息（Frontmatter，我也不知道怎么翻译）部分里可能出现各种乱七八糟的属性，最重要的就是`title`、`date`和`draft`，其他属性不认识都可以删，如果想认识一下可以在主题文档里搜`frontmatter`。
...
如果文章中有HTML代码，比如用`<br>`在表格里换行、`<img width="50">`来插入调整尺寸的图片、`<p align="right">`来给引言作者右对齐，在生成网站时会报警告信息，看到不舒服。考虑到自己写的文章里只会使用非常简单、基本安全的HTML标记，可以在站点配置文件中添加以下内容来允许渲染器（Hugo默认用的Goldmark）渲染文章里的HTML代码：
```toml
[markup.goldmark.renderer]
  unsafe = true
```
如果还是希望保留这个安全考虑，只是不想看见警告，可以只添加下面这一行来不显示相关警告：
```toml
...
最终，我选择在不省略环节的同时，逐渐省略了每个环节里我觉得直接点开链接就能搞定的细节，由读者自己点开链接、参考我认为已经足够完善的文档去填补。我希望对这篇文章的读者而言，锻炼独立解决问题的能力远比机械地照着步骤搭建完网站更重要。

如果你在跟随本文操作时遇到了障碍，恭喜你，这正是锻炼你真正能力的开始。欢迎将你的探索过程和结果与我分享，虽然我还没放联系方式。
