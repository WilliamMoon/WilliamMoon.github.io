...
写博客，主要有两种选择：使用博客平台（如CSDN、掘金等）或者自建博客网站。

“**不怎么麻烦就能获得的自由**”这点就足够让我向自建博客的方向前进，前人撰写的完善的部署文档让我的前进没有阻碍。

从另一个角度来说，我简单看了下CSDN、cnblogs、ITeye、51CTO、掘金这几个平台，不选择博客平台的原因大概是这么三点：
1. 平台出于稳定考虑通常**不会给用户多少操作空间**，而我的探索欲还是挺强的，即使只是博客，我也希望借之尝试各种各样的技术；
2. 平台**追求商业价值**的手段会影响我纯粹专注地构建博客；
3. 博客**平台氛围**和我设想中这个博客的气质不太搭。

**平台最大的好处是自带流量**，便于在技术圈建立影响力，但我暂时没有需求。我是为自己而写，比起技术博客，我的博客定位更像是公开日记，写点技术是因为我脑子里刚好在想技术，而不是准备靠这篇文章分享技术心得。
...
考虑到没有环境依赖，我直接选择了Hugo，电脑上少装一个环境是一个，而且Node.js和Ruby环境在Windows上都有点臃肿而难以管理。

> 不过总结网友评论，最后要完全满足自己需求都要搞最重型的WordPress，二十多年的插件生态、60.5%的CMS市场占有率（我现查的[W3Techs](https://w3techs.com/technologies/details/cm-wordpress)今日最新数据）确实不是吹的。

## 安装Hugo
版本上我直接选择Hugo Extended，因为相比Standard版基本没有额外成本。有这个优点，具体的好处比如支持Sass从而支持更多主题、支持WebP图像处理都不用细看了。

我同时在使用多台电脑，Windows 10/11和macOS系统都有，并且每个系统都安装了包管理器。在有包管理器的情况下，每个系统安装Hugo都非常简单，一个命令搞定，非常适合跟不知道包管理的人显摆。其他系统和其他安装方式参见[官方文档](https://gohugo.io/installation/)。
...
安装完后，会提示新开一个shell，手动`source`或者`refreshenv`来刷新环境变量也行。执行如下命令确认安装成功。
```sh
hugo version
```
...
> `git submodule`比有些主题的文档让在themes目录下`git clone`更好，因为前者能让主题和站点的版本独立管理，便于单独更新主题。
...
根据主题不同，文章开头自动生成的元信息（frontmatter，查了下没看到统一的翻译方法）部分里可能出现各种乱七八糟的属性，最重要的就是`title`、`date`和`draft`，其他属性不认识都可以删，如果想认识一下可以在主题文档里搜`frontmatter`。

写完想发布时，官方是说把`draft`属性改为`false`（当然[文档](https://gohugo.io/getting-started/usage/#draft-future-and-expired-content)里还提到了`date`到了、`expiryDate`没到的要求，不过正常使用不会不满足这些要求），但我直接删了这个属性也可以发布。我觉得大概率不会出问题，出问题再说，反正这个网站我还要大改。
...
hugo server -D
```
启动本地预览服务器，`-D`参数表示包含草稿文章。启动完成后访问显示的URL即可预览网站。

## 部署
采用GitHub Pages部署，简单来说修改站点配置、增加工作流配置，push到仓库后在GitHub里设置用Action部署即可，详见[官方文档](https://gohugo.io/host-and-deploy/host-on-github-pages/)。
...
如果文章中有HTML代码，比如用`<br>`在表格里换行、`<img width="50">`来插入调整尺寸的图片、`<p align="right">`来给引言作者右对齐，在生成网站时会报警告信息，看着让人不舒服。
考虑到自己写的文章里只会使用非常简单、基本安全的HTML标记，在站点配置文件中添加以下内容来允许渲染器（Hugo默认用的Goldmark）渲染文章里的HTML代码：
```toml
...
```
当然最好还是不直接用HTML，相关问题的解决方案等以后遇到再探索吧，这次遇到是Markdown渲染测试那篇文章里在试HTML。
